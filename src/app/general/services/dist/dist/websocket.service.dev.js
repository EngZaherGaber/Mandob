...
function () {
  ...
  WebsocketService.prototype.startConnection = function (userId) {
    ...
    this.hubConnection = new signalR.HubConnectionBuilder().withUrl('/notificationHub').withAutomaticReconnect().build();
    this.hubConnection.start().then(function () {
      console.log('✅ SignalR Connected');

      _this.sendMessageToServer('OnNotification', {
        type: 'login',
        value: {
          Sec: _this.hashUserId(userId)
        }
      });
    })["catch"](function (err) {
      return console.error('❌ SignalR Error:', err);
  ...
  WebsocketService.prototype.sendMessageToServer = function (method, payload) {
    ...
    (_a = this.hubConnection) === null || _a === void 0 ? void 0 : _a.invoke(method, payload)["catch"](function (err) {
  ...
  WebsocketService = __decorate([core_1.Injectable({
    providedIn: 'root'
  })], WebsocketService);

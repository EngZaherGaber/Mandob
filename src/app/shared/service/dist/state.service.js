...
var StateService = /** @class */ (function () {
    function StateService(wsSrv, router, breakpointObserver, userState, platformId) {
        var _this = this;
        this.wsSrv = wsSrv;
        this.router = router;
        ...
        this.taskCount = core_1.computed(function () { return _this.state().taskCount; });
    ...
    StateService.prototype.connectWithTaskCount = function () {
        var _this = this;
        // Refresh every 10 seconds
        rxjs_1.interval(10000)
            .pipe(rxjs_1.switchMap(function () {
            return _this.userState.authSrv.getPendingTasksCount().pipe(
            // If error happens, retry after 10s
            rxjs_1.retryWhen(function (errors) {
                return errors.pipe(rxjs_1.delay(10000));
            }), 
            // Handle errors gracefully (avoid breaking stream)
            rxjs_1.catchError(function (err) {
                console.error('Error fetching task count:', err);
                return rxjs_1.of({ data: 0 }); // return fallback value
            }));
        }))
            .subscribe(function (res) {
            _this.state.update(function (prev) { return (__assign(__assign({}, prev), { taskCount: res.data })); });
        });
    };
    ...
    StateService.prototype.clearNotification = function () {
        this.state.update(function (prev) { return (__assign(__assign({}, prev), { notficiations: [] })); });
    };
    StateService.prototype.addNotification = function (notification) {
        var arr = __spreadArrays(this.notficiations(), [notification]);
        var unique = Array.from(new Map(arr.map(function (item) { return [item.id, item]; })).values());
        this.state.update(function (prev) { return (__assign(__assign({}, prev), { notficiations: unique })); });
    };
